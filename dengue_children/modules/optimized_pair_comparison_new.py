...
def pair_comparison(adata, cell_types, gene_cut_off, log1p=False):    
    import random
    ress = pd.DataFrame([])
    log2FC = pd.DataFrame([])
...
#         ct = cell_type
#         if ct_obs == 'cell_subtype_New':
#             ct = adata_ct.obs['cell_type_new'].unique()[0]
#         ad = adata[adata.obs['cell_type_new'] == ct]

        for ID in IDs:
            if adata_ct[adata_ct.obs[p_ID] == ID].obs.shape[0] < [3, 5][ct_obs == 'cell_type_new']:
                IDs.remove(ID)
        ####### 
